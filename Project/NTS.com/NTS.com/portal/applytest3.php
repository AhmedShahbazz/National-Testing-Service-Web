...
if (isset($_POST['submit'])) {
    ...
    $sql = "INSERT INTO apps (email , first_name, last_name, test_name, phone_number, address, profile_pic, resume) 
            VALUES ('$email' , '$first_name', '$last_name', '$test_name', '$phone_number', '$address', '$profile_pic', '$resume')";
$name = $first_name . " " . $last_name;
    if (mysqli_query($conn, $sql)) {
        echo ".";
        header("Location: ./in.php?name=$name&no=$phone_number&testName=$test_name&tblName=apps");
    } else {
...
.logout-btn:hover {
  ...
  <h1>Graduate Assessment Test (GAT Subject 2023)</h1> 
  <a href="logout.php" class="logout-btn">Logout</a>
    ...
    <form method="post" action="" enctype="multipart/form-data">
      ...
      <label for="profile_pic">Upload Your Pervious Marks Sheet:</label>
      <input type="file" id="profile_pic" name="profile_pic" accept="image/*" required><br>

      <label for="resume">Upload Your Pervious Marks Sheet:</label>
